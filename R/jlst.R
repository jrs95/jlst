...
#' @param type type of test (default: 1 [Breusch-Pagan variance test]; options: 1 [Breusch-Pagan variance test], 2 [Glejser variance test], 3 [robust version of test 1], 4 [robust version of test 2]).
...
jlsp <- function(y, x, covar=NULL, covar.var=FALSE, var.type=1){
  ...
  test <- anova(ols0,ols)[2,c(3,5,6)]; names(test) <- c("DF", "F", "P")
